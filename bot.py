...
def process_cycle(context: CallbackContext):
    ...
    # ONLY PROCESS AT EXACT 00 SECONDS (START OF NEW MINUTE)
    if current_second != 0:
        return
    
    # PREVENT DUPLICATE PROCESSING IN SAME MINUTE
    ...
    # CURRENT ISSUE (JUST ENDED - THIS IS THE ONE WE PREDICTED LAST MINUTE)
    ...
    # NEXT ISSUE (TO PREDICT FOR NEXT MINUTE)
    ...
    for user_id in list(users.keys()):
        ...
        # ============ STEP 1: SEND RESULT FOR CURRENT ISSUE ============
...
📊 *Stats:* W:{user_data['user_wins']} L:{user_data['user_losses']}"""
                    ...
                    new_mult = multiplier * 3
                    user_data["multiplier"] = new_mult
                    user_data["user_losses"] += 1
                    user_data["loss_streak"] += 1
...
📊 *Stats:* W:{user_data['user_wins']} L:{user_data['user_losses']}"""
        ...
        # ============ STEP 2: SEND PREDICTION FOR NEXT ISSUE ============
        prediction_key = f"{user_id}_{next_issue}"
        
        if prediction_key not in sent_predictions:
            # Get history for prediction (use all available data)
            ...
            # Store prediction for this issue
...
def cleanup_task(context: CallbackContext):
    ...
    print(f"🧹 Cleanup: Results cache: {len(sent_results)}, Predictions cache: {len(sent_predictions)}")
...
def main():
    ...
    print("✅ L1-L13 Weighted Prediction Logic (13 Logics)")
    print("✅ EXACT Minute Cycle (Process only at 00 seconds)")
    print("✅ NO SPAM - Only 2 messages per minute per user")
    print("✅ PERFECT Issue Mapping: Prediction(N) → Result(N)")
    print("✅ Duplicate Prevention Enabled")
    print("=" * 70)
    
    # Check for image files
    for img in ["win.jpg", "loss.jpg", "big.jpg", "small.jpg"]:
        if os.path.exists(img):
            print(f"✅ {img} found")
        else:
            print(f"⚠️ {img} NOT found (text-only messages will be sent)")
    ...
    if job_queue:
        ...
        print("✅ Cleanup: Every 1 hour")
    
    print("\n🚀 Bot is running!")
    print("📱 Commands: /start, /stop")
    print("⏰ Cycle: Every minute at exactly 00 seconds")
